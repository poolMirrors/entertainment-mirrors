...
public class MqSender {
     ...
     * 发送库存扣减
     *
     * @param seckillVoucher 扣减库存
     * @param reliable     是否保证可靠传输模式
     */
    public void sendRepertoryMessage(SeckillVoucher seckillVoucher, boolean reliable) {
        log.info("发送消息：" + seckillVoucher);
        // 1.若要保证可靠传递
        if (reliable) {
            // 设置交换机处理消息的模式，交换器无法根据自身类型和路由键找到一个符合条件的队列时的处理方式。true：RabbitMQ会调用Basic.Return命令将消息返回给生产者
            rabbitTemplate.setMandatory(true);
            ...
            rabbitTemplate.setConfirmCallback(((correlationData, ack, cause) -> {
                ...
                if (!ack) {
                    log.error("消息发送失败，错误原因：{}，再次发送。", cause);
                    rabbitTemplate.convertAndSend(MQConstants.SECKILL_EXCHANGE, MQConstants.SECKILL_ROUTING_KEY, seckillVoucher);
                }
            }));
            //（2）设置退回函数，当 exchange 将消息发送到 queue 失败时，自动将消息退回给 publisher
            rabbitTemplate.setReturnsCallback((returnedMessage -> {
        ...
        // 2.发送消息，默认消息持久化
        rabbitTemplate.convertAndSend(MQConstants.REPERTORY_EXCHANGE, MQConstants.REPERTORY_ROUTING_KEY, seckillVoucher);
    }
}
